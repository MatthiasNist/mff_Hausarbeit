...
k = params.Constant % offset labeled "k"

%% Estimating "conditional-means":

object_specif_mu = arima('ARLags',1,'Variance',garch(1,1))

[params_mu output] = estimate(object_specif_mu, dbk_returns_vec)
print(params_mu,output)

constant = params_mu.Constant
ar_1 = params_mu.AR
garch_val = params.GARCH{1} % GARCH-parameter
arch_val = params.ARCH{1}   % ARCH-parameter
k = params.Constant % offset labeled "k"
...
for ii=1:numel(logRets)
    retrieveMus(ii) = constant + ar*logRets(ii-1) + ;
end
